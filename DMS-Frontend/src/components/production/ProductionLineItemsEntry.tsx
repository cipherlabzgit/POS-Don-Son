...
export interface ProductionLineItemsEntryProps {
  products: ProductionLineProduct[];
  productionSections: ProductionSection[];
  items: ProductionLineItem[];
  onItemsChange: (items: ProductionLineItem[]) => void;
  primaryColor?: string;
  enableExcelImport?: boolean;
}

function resolveAutoSection(product: ProductionLineProduct): string {
...
export default function ProductionLineItemsEntry({
  products,
  productionSections,
  items,
  onItemsChange,
  ...
  const [qtyField, setQtyField] = useState<'planned' | 'produced' | null>(null);
  const [qtyDraft, setQtyDraft] = useState('');
  const [detailsOpen, setDetailsOpen] = useState(false);
  const [selectedProduct, setSelectedProduct] = useState<ProductionLineProduct | null>(null);
  ...
  const updateRowQuantity = useCallback(
    (index: number, field: 'plannedQty' | 'producedQty', raw: number) => {
  ...
  const addProduct = useCallback(
    (product: ProductionLineProduct) => {
      ...
      if (existingIndex >= 0) {
        ...
        pendingQtyFocusKey.current = `${existingIndex}-planned`;
        setQuery('');
      ...
      // Add with default quantities
      const newIndex = items.length;
      const newItem: ProductionLineItem = {
      ...
      pendingQtyFocusKey.current = `${newIndex}-planned`;
  ...
  const bumpRowQuantity = (index: number, field: 'plannedQty' | 'producedQty', delta: number) => {
    setQtyEditRow(null);
    const row = items[index];
    updateRowQuantity(index, field, row[field] + delta);
  };

  const handleQtyFocus = (index: number, field: 'plannedQty' | 'producedQty') => {
    const row = items[index];
    setQtyEditRow(index);
    setQtyField(field === 'plannedQty' ? 'planned' : 'produced');
    setQtyDraft(formatQtyValue(row[field]));
  };

  const handleQtyBlur = useCallback(
    (index: number, field: 'plannedQty' | 'producedQty', value: string) => {
      const row = items[index];
      if (!row) return;
      const raw = value.trim().replace(/,/g, '');
      const num = parseFloat(raw);
      if (Number.isNaN(num) || num < 0) {
        updateRowQuantity(index, field, row[field]);
      } else {
        updateRowQuantity(index, field, num);
      }
      setQtyEditRow(null);
      setQtyField(null);
    },
  ...
  const totalPlanned = useMemo(
    () => items.reduce((sum, row) => sum + row.plannedQty, 0),
    [items]
  );

  const totalProduced = useMemo(
    () => items.reduce((sum, row) => sum + row.producedQty, 0),
    [items]
  );
  ...
  const handleImportUpload = async () => {
    ...
    toast.info('Excel import for production will be implemented');
    setImportOpen(false);
    setImportFile(null);
  };
    ...
    <div className="space-y-4">
      ...
      >
                      ...
                      <td className="px-3 py-2 text-right">
                        <div className="inline-flex items-center justify-end gap-1">
                          <button
                            ...
                            onClick={() => bumpRowQuantity(index, 'plannedQty', -step)}
                            title="Decrease"
                          >
                          ...
                          <input
                            ref={(el) => {
                              if (el) qtyInputRefs.current.set(`${index}-planned`, el);
                              else qtyInputRefs.current.delete(`${index}-planned`);
                            }}
                            type="text"
                            inputMode="decimal"
                            ...
                            onFocus={(e) => {
                              handleQtyFocus(index, 'plannedQty');
                              e.currentTarget.select();
                            }}
                            value={
                              qtyEditRow === index && qtyField === 'planned'
                                ? qtyDraft
                                : formatQtyValue(row.plannedQty)
                            }
                            onChange={(e) => {
                              ...
                              const num = parseFloat(raw);
                              if (!Number.isNaN(num) && num > 0) {
                                updateRowQuantity(index, 'plannedQty', num);
                              }
                            }}
                            onBlur={(e) => handleQtyBlur(index, 'plannedQty', e.currentTarget.value)}
                            onKeyDown={(e) => {
                              if (e.key === 'Enter') {
                                e.preventDefault();
                                e.stopPropagation();
                                ...
                                el.blur();
                                // Move focus to produced quantity field
                                const producedKey = `${index}-produced`;
                                requestAnimationFrame(() => {
                                  const producedEl = qtyInputRefs.current.get(producedKey);
                                  if (producedEl) {
                                    producedEl.focus();
                                    producedEl.select();
                                  }
                      ...
                      <td className="px-3 py-2 text-right">
                        <div className="inline-flex items-center justify-end gap-1">
                          <button
                            ...
                            onClick={() => bumpRowQuantity(index, 'producedQty', -step)}
                            title="Decrease"
                          >
                            <Minus className="h-4 w-4" />
                          </button>
                          <input
                            ref={(el) => {
                              if (el) qtyInputRefs.current.set(`${index}-produced`, el);
                              else qtyInputRefs.current.delete(`${index}-produced`);
                            }}
                            type="text"
                            inputMode="decimal"
                            className="h-8 w-[4.5rem] rounded-md border px-1 text-center text-sm tabular-nums outline-none focus:outline focus:outline-2 focus:outline-offset-0"
                            ...
                            onFocus={(e) => {
                              handleQtyFocus(index, 'producedQty');
                              e.currentTarget.select();
                            }}
                            value={
                              qtyEditRow === index && qtyField === 'produced'
                                ? qtyDraft
                                : formatQtyValue(row.producedQty)
                            }
                            onChange={(e) => {
                              const v = e.target.value;
                              setQtyEditRow(index);
                              setQtyField('produced');
                              setQtyDraft(v);
                              const raw = v.trim().replace(/,/g, '');
                              if (raw === '' || raw === '.' || raw === '-') return;
                              const num = parseFloat(raw);
                              if (!Number.isNaN(num) && num >= 0) {
                                updateRowQuantity(index, 'producedQty', num);
                              }
                            }}
                            onBlur={(e) => handleQtyBlur(index, 'producedQty', e.currentTarget.value)}
                            onKeyDown={(e) => {
                              if (e.key === 'Enter') {
                                e.preventDefault();
                                const el = e.target as HTMLInputElement;
        ...
        >
          <span className="text-sm font-semibold tabular-nums" style={{ color: 'var(--foreground)' }}>
            {items.length} line{items.length === 1 ? '' : 's'} · Planned: {totalPlanned.toFixed(2)} · Produced:{' '}
            <span style={{ color: '#dc2626' }}>{totalProduced.toFixed(2)}</span>
          </span>
        </div>
      </div>
      ...
      {enableExcelImport && (
        <Modal
          ...
          title="Import Excel — Production items"
          size="lg"
        >
          <div className="space-y-6">
            <div>
              ...
              <p className="mb-3 text-xs" style={{ color: 'var(--muted-foreground)' }}>
                Add one row per line with columns <strong>ItemCode</strong>,{' '}
                <strong>SectionId</strong>, <strong>PlannedQty</strong>, and{' '}
              ...
              <Button
                ...
                variant="primary"
                onClick={() => {
                  toast.success('Template download will be implemented');
                }}
              >
