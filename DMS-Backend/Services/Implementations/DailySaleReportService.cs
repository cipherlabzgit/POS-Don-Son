...
public sealed class DailySaleReportService : IDailySaleReportService
{
    ...
    static DailySaleReportService()
    {
        QuestPDF.Settings.License = LicenseType.Community;
    }

    public DailySaleReportService(
    ...
    public async Task<IReadOnlyList<DailySaleReportShowroomOptionDto>> ListActiveShowroomsAsync(
        CancellationToken cancellationToken = default)
    {
        return await _context.Outlets
            ...
            .ThenBy(o => o.Code)
            .Select(o => new DailySaleReportShowroomOptionDto
            {
                Id = o.Id,
                Code = o.Code,
                Name = o.Name,
    ...
    public async Task<DailySaleReportDto> BuildReportAsync(
        Guid outletId,
        DateOnly reportDate,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (outlet == null)
            throw new InvalidOperationException("Showroom not found or inactive.");
        ...
        var rows = rawLines
            ...
            .Select(g =>
            {
                var first = g.First();
                var p = first.Product;
                return new DailySaleReportRowDto
                {
        ...
        var totals = new DailySaleReportTotalsDto
        {
    ...
    public byte[] RenderPdf(DailySaleReportDto report)
    {
        ...
        Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Header().Column(c =>
                {
                    ...
                    c.Item().PaddingTop(2).Text($"Showroom: {report.OutletCode} — {report.OutletName}")
                        .FontSize(10);
                    ...
                    c.Item().PaddingTop(4).Text("Approved POS sale lines for this showroom, aggregated by product (system balance).")
                        .FontSize(8)
                        .Italic()
    ...
    public byte[] RenderExcel(DailySaleReportDto report)
    {
        using var workbook = new XLWorkbook();
        var ws = workbook.AddWorksheet("Daily Sale");
        ...
        ws.Cell(3, 1).Value = "Showroom:";
        ws.Cell(3, 2).Value = $"{report.OutletCode} — {report.OutletName}";
        ws.Range(3, 2, 3, 6).Merge();

        ws.Cell(4, 1).Value = "Sales date:";
        ws.Cell(4, 2).Value = report.ReportDate.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture);
