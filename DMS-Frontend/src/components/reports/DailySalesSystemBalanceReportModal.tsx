...
import {
  dailySalesSystemBalanceReportApi,
  type DailySalesSystemBalanceReport,
} from '@/lib/api/reports-daily-sales-system-balance';
import { todayISO } from '@/lib/date-restrictions';
import { formatSlDateTime } from '@/lib/sri-lanka-time';
import Button from '../ui/button';

interface DailySalesSystemBalanceReportModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function DailySalesSystemBalanceReportModal({
  isOpen,
  onClose,
  minReportDate,
  canBypassReportDayEnd,
}: DailySalesSystemBalanceReportModalProps) {
  const [reportDate, setReportDate] = useState(todayISO());
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [preview, setPreview] = useState<DailySalesSystemBalanceReport | null>(null);
  ...
  const loadPreview = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await dailySalesSystemBalanceReportApi.getJson(reportDate);
      setPreview(data);
    } catch (e) {
      setPreview(null);
  ...
  const download = useCallback(
    async (format: 'pdf' | 'xlsx') => {
      ...
      setLoading(true);
      try {
        await dailySalesSystemBalanceReportApi.download(reportDate, format);
      } catch (e) {
            ...
            <tbody>
              {preview.rows.map((r) => (
                <tr key={r.rowNo} className="border-t" style={{ borderColor: 'var(--border)' }}>
                  ...
                  <td className="px-2 py-1.5 text-right tabular-nums">{dailySalesSystemBalanceReportApi.fmtQty(r.totalQuantity)}</td>
                  <td className="px-2 py-1.5 text-right tabular-nums">{dailySalesSystemBalanceReportApi.fmtMoney(r.totalAmount)}</td>
            ...
            <tfoot>
              <tr className="border-t font-semibold" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--muted)' }}>
                ...
                <td className="px-2 py-2 text-right tabular-nums">
                  {dailySalesSystemBalanceReportApi.fmtQty(preview.totals.totalQuantity)}
                </td>
                <td className="px-2 py-2 text-right tabular-nums">
                  {dailySalesSystemBalanceReportApi.fmtMoney(preview.totals.totalAmount)}
                </td>
