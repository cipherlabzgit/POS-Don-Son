# Don & Sons DMS — deploy with LOCAL PostgreSQL (pgAdmin) + Docker app services
# Creates empty database on host PG if needed; backend migrations create all tables.
$ErrorActionPreference = "Stop"
$Root = Split-Path -Parent $PSScriptRoot
Set-Location $Root

Write-Host "=== Don & Sons DMS — Local PostgreSQL (pgAdmin) + Docker ===" -ForegroundColor Cyan
...
# Find psql (local PostgreSQL on Windows)
...
if (-not (Test-Path $psql)) {
    ...
    else { throw "psql not found. Install PostgreSQL 18 and ensure the service is running." }
}
...
if ($LASTEXITCODE -ne 0) {
    throw "Cannot connect to local PostgreSQL. Check POSTGRES_USER/POSTGRES_PASSWORD in .env and that postgresql service is running."
}
...
Write-Host ""
Write-Host "Starting Docker (backend, frontend, pos) — using HOST PostgreSQL, NOT Docker postgres ..." -ForegroundColor Cyan
...
if ($healthy) {
    ...
    Write-Host "Backend not healthy yet. Check: docker compose -f docker-compose.yml -f docker-compose.local-pg.yml logs backend" -ForegroundColor Yellow
}

$hostName = Get-EnvValue "CLIENT_HOST" "localhost"
